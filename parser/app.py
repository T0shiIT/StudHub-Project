...
def _records_from_frame(frame: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for idx, row in cleaned.iterrows():
        score = sum(1 for value in row.values if value is not None and str(value).strip() != "")
        if score > best_score:
            best_score = score
            header_row_idx = idx
    ...
    header_values = [_normalize_value(v) for v in cleaned.loc[header_row_idx].values]
