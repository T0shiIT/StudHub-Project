...
public class CourseService {
    ...
    public CourseDto getCourse(Long courseId, Long userId) {
        ...
        if (!canViewCourse(course, user, enrollment)) {
    ...
    // ------------------------------------------------------------------
    // Создание / редактирование / архив / удаление
    // ------------------------------------------------------------------
    @Caching(evict = {
    ...
    // ------------------------------------------------------------------
    // Запись / отказ от курса
    // ------------------------------------------------------------------
    @Caching(evict = {
            @CacheEvict(value = "courses", allEntries = true),
            @CacheEvict(value = "myCourses", allEntries = true),
            @CacheEvict(value = "course", key = "#courseId + ':' + #userId")
    })
    @Transactional
    public CourseEnrollmentDto selfEnroll(Long courseId, Long userId) {
        Course course = requireCourse(courseId);
        if (!course.isEnrollmentOpen() || course.getStatus() != CourseStatus.ACTIVE) {
    ...
    // ------------------------------------------------------------------
    // Управление участниками (для владельца / ADMIN)
    // ------------------------------------------------------------------
    @Caching(evict = {
            @CacheEvict(value = "course", key = "#courseId + ':' + #currentUserId"),
    ...
    public CourseEnrollmentDto addMember(Long courseId, AddMemberRequest req, Long currentUserId) {
        ...
        if (enrollmentRepo.existsByCourseIdAndUserId(courseId, target.getId())) {
    ...
    public void removeMember(Long courseId, Long targetUserId, Long currentUserId) {
        ...
        if (!isSelf && !canManageCourse(course, manager)) {
    ...
    private boolean canViewCourse(Course course, User user, CourseEnrollment enrollment) {
