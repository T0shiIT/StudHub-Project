...
def _csrf(session):
    session.get(f"{BASE_URL}/api/user")
    return session.cookies.get("XSRF-TOKEN")


class TestLatestSchedule:

    def test_authenticated_can_get_latest(self, student):
        r = student["session"].get(
            f"{BASE_URL}/api/schedule/latest",
            timeout=10
        )

        assert r.status_code in (200, 404)

    def test_unauthenticated_latest(self):
        r = requests.get(
            f"{BASE_URL}/api/schedule/latest",
            timeout=10
        )
...
class TestScheduleUploads:

    def test_get_uploads_requires_auth(self):
        r = requests.get(
            f"{BASE_URL}/api/schedule/uploads",
            timeout=10
        )

        assert r.status_code == 401

    def test_authenticated_can_get_uploads(self, admin):
        r = admin["session"].get(
            f"{BASE_URL}/api/schedule/uploads",
            timeout=10
        )
    ...
    def test_admin_can_upload_empty_file_fails(self, admin):
        csrf = _csrf(admin["session"])

        r = admin["session"].post(
            f"{BASE_URL}/api/schedule/upload",
            files={
                "file": (
                    "empty.xlsx",
                    io.BytesIO(b""),
    ...
    def test_delete_nonexistent_schedule(self, admin):
        csrf = _csrf(admin["session"])

        r = admin["session"].delete(
            f"{BASE_URL}/api/schedule/uploads/99999999",
            headers={"X-XSRF-TOKEN": csrf},
            timeout=10
        )
