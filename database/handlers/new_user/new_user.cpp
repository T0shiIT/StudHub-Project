    ...
    void register_new_user_handler(crow::SimpleApp& app) {
        ...
        ([](const crow::request& req) {
            try {
                ...
                std::cout << "[DB] Registered user " << email << " (id=" << userid << ")" << std::endl;
    ...
    void register_oauth_user_handler(crow::SimpleApp& app) {
        ...
        ([](const crow::request& req) {
            try {
                ...
                std::cout << "[DB] Synced OAuth user " << email << " (id=" << userid << ")" << std::endl;

                json res;
                res["status"] = "ok"; res["id"] = userid; res["role"] = currentRole; 
