...
namespace handlers {
    ...
    void set_role(crow::SimpleApp& app) {
        ...
        ([](const crow::request& req) {
            ...
            // --- Проверка прав вызывающего (ADMIN) ---
            ...
            // Смена роли происходит для пользователя, ID которого указан в X-User-Id
            // (в текущей реализации Java отправляет тот же ID, что и запрашивающий).
            // При необходимости можно добавить параметр target_user_id.
            try {
                ...
                std::cout << "[DEBUG] Role for user " << user_id
                          << " changed: " << old_role << " -> " << target_role << std::endl;
